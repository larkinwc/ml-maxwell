...
def main():
    llm=LLM(model=p, tokenizer="Qwen/Qwen3.5-9B", trust_remote_code=True, dtype="float16",
            max_model_len=320, max_num_seqs=1, gpu_memory_utilization=0.95,
            enforce_eager=True, tensor_parallel_size=TP)
    # warmup to cover triton JIT for decode shapes
    llm.generate(["Hello there friend"], SamplingParams(temperature=0.0,max_tokens=16))
    sp=SamplingParams(temperature=0.0, max_tokens=128)
    t0=time.time()
    out=llm.generate(["The capital of France is"], sp)
    dt=time.time()-t0
    n=len(out[0].outputs[0].token_ids)
    print(f"RESULT TP={TP}: {n} tokens in {dt:.2f}s = {n/dt:.2f} tok/s")
    print("TEXT:", repr(out[0].outputs[0].text[:160]))
